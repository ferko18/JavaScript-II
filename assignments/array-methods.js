...
let largeShirts = [];
largeShirts=runners.filter(currentValue=>currentValue.shirt_size ='L')
...
// Problem 1: highest donation? 
let DonationList =[];
DonationList=runners.map(currentValue=>currentValue.donation); //all donations list 
DonationList.sort(function(m,n){return n-m})// sort in descending order 
console.log(DonationList[0]); // log the first item

// Problem 2: filter those who donated above 200
let bigDonation = [];
bigDonation=runners.filter(currentValue=>currentValue.donation >200)

console.log(bigDonation);
console.log(bigDonation.length)//16 of them donated >200

// Problem 3: convert every donations to euros 
let Donationeuro =[];
Donationeuro=runners.map(currentValue=>currentValue.donation*0.89);
console.log(Donationeuro);